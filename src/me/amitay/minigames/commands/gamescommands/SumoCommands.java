...
public class SumoCommands implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 1) {
            p.sendMessage(Utils.getFormattedText("&cCorrect usage: /sumo set [spawnplayer1] / [spawnplayer2] / [spawn] / [spectators] / [minplayers] / [maxplayers] / [timetostart]"));
            return true;
        }
        if (args.length == 2){
            if (args[0].equalsIgnoreCase("set")){
                ...
                if (args[1].equalsIgnoreCase("spawn")){
                    pl.getConfig().set("minigames.games.sumo.spawn", p.getLocation());
                    pl.saveConfig();
